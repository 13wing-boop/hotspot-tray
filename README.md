...
- **20 KB 단일 exe** — 설치 불필요, 외부 DLL·아이콘 파일 없음, .NET Framework 4.x(윈도우 기본 탑재)만 있으면 동작
- 윈도우 로그온 시 자동 실행 + 핫스팟 자동 켜기
- **꺼짐 방지** — 내가 끄기 전까지 핫스팟이 스스로 꺼지지 않음
...
| 🟠 주황 | 전환 중 | `핫스팟 전환 중...` / `핫스팟 다시 켜는 중... · JSH-SP 6054` |
...
- **꺼짐 방지 (내가 끌 때까지 유지)** — 아래 [꺼짐 방지](#꺼짐-방지) 참고 (기본값: 켜짐)
...
기본으로 켜져 있습니다. **트레이에서 직접 끄기 전까지 핫스팟이 스스로 꺼지지 않게** 두 겹으로 막습니다.

**1. 무연결 타임아웃 해제** — 윈도우는 연결된 기기가 없으면 약 5분 뒤 핫스팟을 끕니다(설정 앱의 "전원 절약").
`NetworkOperatorTetheringManager.DisableNoConnectionsTimeout()` 으로 이 동작 자체를 끕니다.
핫스팟을 껐다 켜면 되살아나는 시스템 전역 설정이라, 켤 때마다 다시 호출합니다.

**2. 감시 후 재점등** — 타임아웃 외의 원인(절전 모드 복귀, Wi-Fi 어댑터 재시작, 인터넷 연결 프로필 변경)으로
꺼지는 경우까지 덮기 위해, 2초 주기 상태 확인에서 "켜져 있어야 하는데 꺼짐"을 발견하면 다시 켭니다.
켤 수 없는 상황(인터넷 연결 없음 등)이면 5초 → 10초 → … 최대 60초로 간격을 늘려 가며 재시도하므로
연결이 없는 동안 CPU를 낭비하지 않습니다. 복구되면 트레이 알림으로 알려주고, 알림은 1분에 한 번으로 제한됩니다.

> **"내가 껐다"의 기준은 트레이입니다.** 트레이 좌클릭이나 메뉴의 "핫스팟 끄기"로 끄면 감시가 해제됩니다.
> 반면 **윈도우 설정 앱에서 끄면 프로그램이 다시 켭니다** — 의도치 않게 꺼진 것과 구분할 방법이 없기 때문입니다.
> 설정 앱에서 끄고 싶다면 이 메뉴를 먼저 꺼 주세요.

이 항목을 끄면 윈도우의 무연결 타임아웃도 원래대로(켜짐) 되돌려 놓습니다.
...
| 꺼짐 방지 | `HKCU\Software\HotspotTray` → `KeepHotspotOn` (DWORD, 기본 1) |
...
- **연결된 기기가 없으면 윈도우가 약 5분 뒤 핫스팟을 자동으로 끕니다.** [꺼짐 방지](#꺼짐-방지)가 이 동작을 API로 직접 해제하므로 설정 앱을 건드릴 필요가 없습니다. (예전에는 설정 → 네트워크 및 인터넷 → 모바일 핫스팟에서 "전원 절약"을 수동으로 꺼야 했습니다.)
- SSID/비밀번호 변경은 윈도우 설정 앱에서 하세요. 이 프로그램은 읽기만 합니다.
- **SSID 숨김(비콘 미방송)은 지원하지 않습니다.** 윈도우 모바일 핫스팟 API 자체에 해당 기능이 없습니다 — `NetworkOperatorTetheringAccessPointConfiguration` 이 노출하는 속성은 `Ssid` / `Passphrase` / `Band` / `AuthenticationKind` 뿐입니다. 레거시 Hosted Network(`netsh wlan set hostednetwork`)에도 숨김 옵션이 없고, 요즘 어댑터 대부분은 이 기능 자체를 지원하지 않습니다(`netsh wlan show drivers` → `Hosted network supported: No`). 보안이 목적이라면 `AuthenticationKind` 를 WPA3 로 올리는 편이 실효가 있습니다.
